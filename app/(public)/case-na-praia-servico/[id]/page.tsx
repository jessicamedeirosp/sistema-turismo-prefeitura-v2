...
export default function CaseNaPraiaServicoDetailPage() {
  const params = useParams()
  const id = params?.id as string

  const { data, isLoading } = useQuery({
    queryKey: ['case-na-praia-servico-detail', id],
    queryFn: async () => {
      const res = await fetch(`/api/public/case-na-praia/${id}`)
      if (!res.ok) throw new Error('Erro ao buscar detalhes do serviço')
      return res.json()
    },
    enabled: !!id,
  ...
  if (!data) {
    return <div className="text-center py-12 text-gray-500">Serviço não encontrado.</div>
  }
