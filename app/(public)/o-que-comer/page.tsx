...
export default function OQueComerPage() {
  ...
  const {
    ...
    queryKey: ['businessPageData'],
    queryFn: async () => {
      const res = await fetch('/api/public/common-pages?page=BUSINESS_FOOD')
  ...
  const {
    ...
    queryKey: ['businesses'],
    queryFn: async () => {
      const res = await fetch('/api/public/o-que-comer')
  ...
  const {
    ...
    queryKey: ['tags'],
    queryFn: async () => {
      const res = await fetch('/api/tags?category=FOOD')
  ...
  interface FilteredBusiness extends Business { }

  let filteredBusinesses: FilteredBusiness[] = businesses.filter((business: Business) => {
    const matchesSearch: boolean = business.name.toLowerCase().includes(searchTerm.toLowerCase())
    const matchesBairro: boolean = selectedBairros.length === 0 || selectedBairros.includes(business.district)
    const matchesTags: boolean =
      selectedTags.length === 0 ||
      (Array.isArray(business.tags) ? business.tags.some((tag: { id: string; name: string; icon?: string }) => selectedTags.includes(tag.name)) : false)
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {businessPageData && (
        <DetailBanner title={businessPageData.title} images={businessPageData.images} details={businessPageData.details} />
      )}
      <FilterBar
      ...
      <section className="py-12">
        <div className="container mx-auto px-4">
          ...
          <ListingGrid
            ...
            onClearFilters={handleClearFilters}
            emptyLabel="Nenhum estabelecimento encontrado com os filtros selecionados."
            hrefBase="/o-que-comer"
          />
