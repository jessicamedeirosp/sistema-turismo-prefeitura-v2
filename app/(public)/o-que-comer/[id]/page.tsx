...
export default function OQueComerDetailPage() {
  const params = useParams();
  const id = params?.id as string;

  const { data, isLoading } = useQuery({
    queryKey: ["o-que-comer-detail", id],
    queryFn: async () => {
      const res = await fetch(`/api/public/o-que-comer/${id}`);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <DetailBanner title={data.name} images={data.images} details={data.details} />
