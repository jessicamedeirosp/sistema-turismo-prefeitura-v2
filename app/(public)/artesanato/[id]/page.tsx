...
export default function ArtesanatoDetailPage() {
  const params = useParams()
  const id = params?.id as string

  const { data, isLoading } = useQuery({
    queryKey: ['artesanato-detail', id],
    queryFn: async () => {
      const res = await fetch(`/api/public/artesanato/${id}`)
      if (!res.ok) throw new Error('Erro ao buscar detalhes do artesanato')
      return res.json()
    },
    enabled: !!id,
  ...
  if (!data) {
    return <div className="text-center py-12 text-gray-500">Artesão não encontrado.</div>
  }
